...
sudo apt-get update
sudo apt-get install -y git unzip s3cmd curl redis-server supervisor g++
...
# update nodejs
su vagrant -c "cd $HOME && source $HOME/.nvm/nvm.sh && nvm install $NVM_VERSION"
su vagrant -c "cd $HOME && source $HOME/.nvm/nvm.sh && nvm use $NVM_VERSION"

# Update npm
su vagrant -c "npm update npm -g"

# Install jacc
su vagrant -c "source $HOME/.nvm/nvm.sh && nvm use $NVM_VERSION && sudo npm install jacc -g"

# Install grunt-cli
su vagrant -c "source $HOME/.nvm/nvm.sh && nvm use $NVM_VERSION && sudo npm install grunt-cli -g"


# Make sure the right node version is used
# vagrant up is affected by this - echo "nvm use $NVM_VERSION" >> $HOME/.profile

# Fix ownership for vagrant
chown vagrant:vagrant -R $HOME

# install nvm for root user, NOTE: grunt and jacc will be install for root this way
if [ ! -f /usr/bin/node ]; then
    echo "/usr/bin/node not found! Copying."
    source $HOME/.nvm/nvm.sh; nvm use $NVM_VERSION; n=$(which node);n=${n%/bin/node}; chmod -R 755 $n/bin/*; sudo cp -r $n/{bin,lib,share} /usr
fi
